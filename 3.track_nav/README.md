...
```bash
# 将 lua 和 launch 文件复制到 cartographer_ros 配置目录
...
| `provide_odom_frame` | `true` | 自动发布 odom→map 变换 |
...
| `static_transform_publisher x4` | 发布静态 TF 变换 (map→odom、map→camera_init、camera_init→base_link) |
...
| 变换 | 发布者 | 说明 |
|------|--------|------|
| `map` → `odom` | `static_transform_publisher` | 恒等变换，始终一致 |
| `map` → `camera_init` | Cartographer | SLAM 实时估计的机器人位姿 |
| `camera_init` → `base_link` | `static_transform_publisher` | 恒等变换，Carter 发布位姿的参考系 |
| `base_link` → `body` | PX4 (MAVROS) | 飞控自身估计的本体位姿 |

> **注意：** `map` 和 `camera_init` 之间有一个**冗余**的 `odom` 帧，这是因为 Cartographer 需要 `provide_odom_frame`。实际上所有位姿信息由 Cartographer 直接发布在 `camera_init` 中，`odom` 帧不做额外里程计融合。
...
**A:** 确保 `livox.lua` 已复制到 `my_carto/src/cartographer_ros/cartographer_ros/configuration_files/` 并重新编译。
