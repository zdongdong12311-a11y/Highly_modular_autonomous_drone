...
int main(int argc, char **argv)
{
    ...
    NN_LOG_INFO("Model input: %d x %d | video source: %d x %d",
                yolo.GetModelInputWidth(), yolo.GetModelInputHeight(), width, height);

    cv::Mat img;
    cv::VideoWriter writer;
    if (record)
    {
        writer = cv::VideoWriter("result.mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), fps,
