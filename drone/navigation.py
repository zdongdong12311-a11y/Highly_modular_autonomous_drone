            ...
            with open('~/point.txt', 'r') as f:
                for line in f:
                    ...
                    if len(parts) >= 4:
                        ...
                        z = float(parts[2])
                        t = float(parts[3])
                        rospy.loginfo("Read waypoint from file: x=%.2f, y=%.2f, z=%.2f, hover=%.2f", x, y, z, t)
                        # 依次导航到各航点并悬停对应时间
                        my_navigation.navigation_target(x, y, z, t)
                    else:
                        rospy.logwarn("Invalid line format in point.txt: %s", line.strip())
        except IOError:
            rospy.logerr("Failed to open point.txt! Please make sure the file exists.")
    else:
