...
> 2. FAST-LIO2 的 TF 坐标系（`camera_init` → `body`）与 Cartographer 的坐标系（`map` → `camera_init`）互补，**不会冲突**，无需修改配置文件。
> 
>    ⚠️ 网上有些教程提到 `lidar_odometry_frame_id`、`body_pose_frame_id` 等参数——这些是社区个人移植版（如 guzhaoyuan 的 fork）添加的功能。**官方 hku-mars 版本没有这些参数**，坐标系名称是硬编码在 `laserMapping.cpp` 中的（`"camera_init"` 和 `"body"`），不能通过 yaml 或命令行修改。
